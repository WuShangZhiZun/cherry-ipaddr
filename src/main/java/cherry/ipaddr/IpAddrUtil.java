...
public class IpAddrUtil {
	 ...
	 */
	public boolean isIpv4Addr(final String addr) {
		if (addr == null) {
			return false;
		}
	 ...
	 */
	public boolean isIpv6Addr(final String addr) {
		if (addr == null) {
			return false;
		}
	 ...
	 */
	public String decompressIpv6Addr(final String addr) {
	 ...
	 */
	public String compressIpv6Addr(final String addr) {
	 ...
	 */
	public BigInteger getIpv4AddrAsNumber(final String addr) {
	 ...
	 */
	public BigInteger getIpv6AddrAsNumber(final String addr) {
	 ...
	 */
	public BigInteger getIpv4AddrMask(final int maskLength) {
		if (maskLength > 32) {
	 ...
	 */
	public BigInteger getIpv6AddrMask(final int prefixLength) {
		if (prefixLength > 128) {
	 ...
	 * @return ビットマスクの数値表現。
	 */
	private BigInteger getIpAddrMask(final int size, final int length) {
		// [考え方]
		// -------- <---------------(size)---------------->
		// -------- <-----(length)----> <-(size - length)->
		// --------+-------------------+--------------------
	 ...
	 */
	public String getIpv4AddrFromNumber(final BigInteger addr) {
	 ...
	 */
	public String getIpv6AddrFromNumber(final BigInteger addr) {
	 ...
	 */
	private Ipv6Fragment makeIpv6Fragment(final String addr) {
	 ...
	 * @return 生成した配列。
	 */
	private String[] padding(final int size, final String[] prefix,
			final String[] suffix) {
	 ...
	 */
	private Range computeMaxNullRange(final String[] field) {
	 ...
	 */
	private void constructDecompressed(final StringBuilder builder,
			final String[] field) {
	 ...
	 *            圧縮表現に結合する際の0値連。
	 */
	private void constructCompressed(final StringBuilder builder,
			final String[] field, final Range range) {
	 ...
	 *            結合するIPv6アドレスの項。
	 */
	private void appendDecompressed(final StringBuilder builder,
			final String text) {
		if (isZero(text)) {
			builder.append("0000");
		} else {
	 ...
	 *            結合するIPv6アドレスの項。
	 */
	private void appendCompressed(final StringBuilder builder, final String text) {
		if (isZero(text)) {
			builder.append("0");
		} else {
			int index = 0;
	 ...
	 */
	private boolean isZero(final String text) {
		if (text == null) {
			return true;
		} else {
